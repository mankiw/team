...
-export([ets_ext_new/1, ets_ext_insert/2, ets_ext_delete/2, get_invite_author_list/1]).
...
-export([msg_sc_enter_hall/0, msg_sc_enter_hall_notice/2, msg_sc_quit_hall_notice/2,
		 msg_sc_team_insert_notice/1, msg_sc_team_delete_notice/1, gold_invite/2,
    ...
    case team_type(Player) of
        ...
        TeamType ->
           ...
           case ets:lookup(?CONST_ETS_TEAM_AUTHOR, {UserId, TeamType}) of
               [] ->
                   ets:insert(?CONST_ETS_TEAM_AUTHOR, get_Author_item(UserId, TeamType)),
                   List = [];
               [Rec] ->
                   ListFrom = Rec#team_author.team_from,
                   Fun = 
                       fun(Id) ->
                               AuthorRec = ets_get_authoer(Id, TeamType),
           ...
           Packet = msg_sc_invite_author_list(List, TeamType),
           misc_packet:send(UserId, Packet)
    end.
...
add_author_from(UserId1, UserId2, TeamType) ->
    ...
    OldList = Rec#team_author.team_from,
    NewList = 
...
set_author_list(Player, UserId1, TeamType, IsChoose) ->
    ...
    OldList = Rec#team_author.team_to,
    case UserId1 > 0 of
        true ->
            case IsChoose of
                true ->
...
get_author_list(Player, Type) ->
    ...
    AuthorList = Rec#team_author.team_to,
    IsGuildAll = Rec#team_author.is_guild_all,
    PacketFormat=
        fun(Id) ->
                {Id}
...
init_invite_info(UserId, Type) ->
    ...
    case mysql_api:select(Keys, game_team_invite_offline,  [{userId, UserId}, {type, Type}]) of
        {ok, [[UserId, Type, TeamTo, TeamFrom, IsGuildAll, LastTime, Count]]} ->
            ?MSG_ERROR("misc:bitstring_to_term(TeamTo) is ~w", [misc:bitstring_to_term(TeamTo)]),
            Rec = #team_author{key = {UserId, Type},
                               times = Count, 
                               team_to = misc:bitstring_to_term(TeamTo), 
                               team_from = misc:bitstring_to_term(TeamFrom), 
                               name = Name,
                               lv = Lv,
                               career = Career,
                               is_guild_all = 
...
cross_join(Player, TeamKey, Password) ->
    ?MSG_ERROR("~w try to join team ~w", [Player#player.user_id, TeamKey]),
    case team_type(Player) of
        0 ->% 未在组队玩法中，无资格加入
            {?error, ?TIP_TEAM_NOT_TEAM_PLAY};
        TeamType ->
                ...
                {?ok, Team} ->
                    case team_serv:gold_invite_call(Team#team.team_pid, UserId) of
                        ?ok ->
                            ...
                            ?MSG_ERROR("TeamPlayer is ~w", [TeamPlayer]),
                            case team_mod:insert_member_to_team3(Team, TeamPlayer) of
                                {?ok, Team2} ->
                                    {
                                     _EtsTeamId, _EtsTeamHall, EtsTeamInfo, _EtsTeamPlayer, _EtsTeamExt
                                    }   = team_api:team_ets(TeamType),
                                    ?MSG_ERROR("Team2 author list  is ~w", [Team2#team.author_list]),
        ...
        _ ->
            case ets:lookup(?CONST_ETS_TEAM_INVITE_CD, {LeaderId, UserId, Type}) of
                ...
                [CdRec] ->
                    ...
                    ?MSG_ERROR("Next time is ~w, Now is ~w", [Next, misc:seconds()]),
                    Next < misc:seconds()
            end
    end.
...
play_over(TeamType, TeamId) ->
	...
	case check_play_over(EtsTeamInfo, EtsTeamPlayer, TeamId) of
		{?ok, Team} ->
            ...
            G = 
                fun(CampPos) when is_record(CampPos, camp_pos) ->
                          case lists:keyfind(Team#team.leader_uid, #team_player.uid, CrossList) of
                              false ->
                                  ?ok;
                              TeamPlayer ->
                                  Index = TeamPlayer#team_player.index_from,
                                  Node = cross_api:get_node(Index),
                                  rpc:cast(Node, player_api, process_send, [Team#team.leader_uid, team_api, remove, CampPos#camp_pos.id])
                          end;
                         (_) -> ?ok
                      end,
...
filter(Player, _EtsTeamInfo, Id) ->
	...
	case Result of
		{?ok, Player3} -> {?ok, Player3};
		{?error, _} -> misc_packet:send_tips(UserId, ?TIP_TEAM_NO_FIT_TEAM),
                       {?ok, Player}
	end.
...
refresh_exit_guild_author(UserId, GuildId, Type) ->
    ...
    Fun = 
        fun({Id, _, _}) ->
                ...
                AuthorToList = AuthorRec#team_author.team_to,
...
broadcast_team(Player, Packet) when is_record(Player, player) ->
	case team_type(Player) of
		...
		TeamType ->
            ...
            case Player#player.team_id of
                {TeamId, ServId} ->
                    ?MSG_ERROR("b22222road team but team not local cast to server ~w broad ", [ServId]),
