...
generate() ->
	FunDatas1 = generate_mcopy(get_mcopy),
	FunDatas2 = generate_mcopy_serial(get_mcopy_serial),
	FunDatas3 = generate_mcopy_list(get_mcopy_list),
	FunDatas4 = generate_mcopy_id(get_mcopy_id),
	FunDatas5 = generate_encounter(get_encounter),
...
%% 副本信息
generate_mcopy(FunName) ->
	McopyDatas = 
        case misc_app:load_file(?DIR_YRL_ROOT ++ "mcopy/mcopy.yrl") of
...
%% 副本系列信息
generate_mcopy_serial(FunName) ->
    McopySerDatas = 
        case misc_app:load_file(?DIR_YRL_ROOT ++ "mcopy/mcopy_serial.yrl") of
            Data when is_list(Data) ->
                Data;
            Data ->
                [Data]
...
generate_mcopy_list(FunName) ->
    ...
    generate_mcopy_list(FunName, McopyDatas).
generate_mcopy_list(FunName, Datas) ->
    Key     = ?null,
    Value   = [MCopy#rec_mcopy_serial.id||MCopy <- Datas],
    When    = ?null,
    {FunName, [{Key, Value, When}]}.

generate_mcopy_id(FunName) ->
	DataList = misc_app:get_data_list("mcopy/mcopy_serial.yrl"),
...
%% 奇遇信息
generate_encounter(FunName) ->
    EncounterDatas = 
        case misc_app:load_file(?DIR_YRL_ROOT ++ "mcopy/encounter.yrl") of
            Data when is_list(Data) ->
                Data;
            Data ->
                [Data]
